    ...
    struct Qv2rayConfig
    {
        ...
        Qv2rayConfig()
            : config_version(QV2RAY_CONFIG_VERSION), tProxySupport(false), logLevel(), autoStartId("null"), groups(), subscriptions(),
              connections(), uiConfig(), apiConfig(), pluginConfig(), kernelConfig(), updateConfig(), toolBarConfig(), inboundConfig(), connectionConfig()
        {
        }

        XTOSTRUCT(O(config_version, tProxySupport, logLevel, uiConfig, pluginConfig, kernelConfig, updateConfig, groups, connections, subscriptions,
